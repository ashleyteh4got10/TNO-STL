...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			{
				float2 BurnCenter = float2(0.5547f, 0.5787f);
				float Aspect = 1.0f;
				float cTime = vTime - 27.f;
				float Radius0 = 0.0f;
				float Speed = 0.15f;
				float JagAmp = 0.01f;
				float Freq = 4.f;
				...
				float RimSigma = 0.15f;
				float HeatAmp = 0.212f;
				float3 EmberColor = float3(0.8, 0.6, 0.4);
				float EmberBoost = 0.55f;
				
				float4 burn1 = ApplyBurn(v.vTexCoord, MapTexture, BurnCenter, Aspect, cTime, Radius0, Speed, JagAmp, Freq, Flow, EdgeWidth, RimWidth, RimSigma, HeatAmp, EmberColor, EmberBoost);
				float4 finalCol = burn1;
				if(vTime > 27.5f)
				{
					BurnCenter = float2(0.5547f, 0.1352f);
					cTime = vTime - 27.5f;
					float4 burn2 = ApplyBurn(v.vTexCoord, MapTexture, BurnCenter, Aspect, cTime, Radius0, Speed, JagAmp, Freq, Flow, EdgeWidth, RimWidth, RimSigma, HeatAmp, EmberColor, EmberBoost);
					// alpha union
					float alpha = max(burn1.a, burn2.a);

					// additive colors for glow intensity
					float3 color = burn1.rgb + (burn2.rgb * saturate((vTime - 27.5) * 3));

					// prevent double-brightening the base image
					// clamp to 1 so it doesn't blow out
					color = saturate(color);

					// mask by alpha
					color *= alpha;

					finalCol = float4(color, alpha);
				}
				return finalCol;
			}
		    return OutColor;
