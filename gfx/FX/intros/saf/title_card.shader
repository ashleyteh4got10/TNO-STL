...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			if(vTime < 2.f || vTime > 8.f){
				return float4(0,0,0,0);
			}
			else if(vTime >= 2.f && vTime < 4.f)
			{
				OutColor.a *= (vTime - 2.f) * 0.5f;
			}
			else if(vTime >= 6.f && vTime < 8.f)
			{
				OutColor.a *= 1.f - ((vTime - 6.f) * 0.5f);
			}
