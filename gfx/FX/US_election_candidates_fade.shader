...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			float firstPortOffset = (firstPort - 1.f)/19.f;
			float secondPortOffset = (firstPort)/19.f;
			if(timeCheck > 5 && timeCheck < 7) {
				firstPortOffset = (5)/19.f;
				secondPortOffset = (threePort - 1.f)/19.f;
			}
			else if(timeCheck > 6 && timeCheck < 8) {
				firstPortOffset = (threePort - 1.f)/19.f;
				secondPortOffset = (penPort - 1.f)/19.f;
			}
			else if(timeCheck > 7 && timeCheck < 9) {
				firstPortOffset = (penPort - 1.f)/19.f;
				secondPortOffset = (lastPort - 1.f)/19.f;
			}
			else if(timeCheck > 8) {
				secondPortOffset = (lastPort - 1.f)/19.f;
